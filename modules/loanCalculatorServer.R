...
# Update your ggplot calls in the server function to include this custom theme.
...
output$loan_plot_balance <- renderPlot({
  ...
  ggplot(loan_summary, aes(x = repay_no, y = balance)) + 
    ...
    scale_y_continuous(labels = scales::dollar_format(prefix = "KES "), expand = c(0, 0)) + 
    custom_ggplot_theme()
}),
...
output$loan_plot_interest_vs_capital <- renderPlot({
  ...
  ggplot(int_cap, aes(x = repay_no, y = value, fill = int_or_cap, color = int_or_cap)) + 
    geom_bar(stat = "identity") + 
    scale_fill_manual(values = c("#4A8DBF", "#BF7C4A")) + 
    scale_colour_manual(values = c("#4A8DBF", "#BF7C4A"), guide = FALSE) +
    labs(x = "Repayment Number", y = "% of Repayment", fill = NULL) + 
    xlim(1, nrow(loan_summary)) +
    scale_x_continuous(expand = c(0, 0)) + 
